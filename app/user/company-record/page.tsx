...
export default function CompanyRecordPage() {
  ...
  const handleDelete = async (profileId: string) => {
    try {
      setLoading(true);
      const result = await deleteCompanyProfile(profileId);
      if (result.success) {
  ...
  return (
    <div>
                ...
                items={[
                  {
                    ...
                    children: renderExpandedContent(profile),
                  },
                ]}
              />
            ))}
      ...
      <CompanyProfileModal
        open={isModalOpen}
        onClose={handleClose}
        initialData={modalMode === "edit" ? selectedProfile : null}
      />
    </div>
  );
}
